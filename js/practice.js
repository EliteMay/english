...
function renderPractice(){
  ...
  const qs=p.questions.slice(currentPage*PAGE_SIZE,(currentPage+1)*PAGE_SIZE);$('#questionList').innerHTML=qs.map((q,i)=>renderQuestion(q,currentPage*PAGE_SIZE+i+1)).join('');renderReviewPanel(qs);bindQuestions();updatePracticeProgress();updateFinishButton();setMode('pen',false);save();setTimeout(()=>{resizeCanvas();renderInk()},20)
}
...
function renderReviewPanel(qs){
  let host=$('#reviewPanel');if(!host){host=document.createElement('div');host.id='reviewPanel';host.className='review-panel';$('.practice-footer').before(host)}
  const pr=progress();if(!pr.reviewMode){host.classList.add('hidden');host.innerHTML='';return}
  host.classList.remove('hidden');host.innerHTML=`<div class="review-panel-head"><div><span>ANSWER CHECK</span><strong>このページの答え合わせ</strong></div><small>紙面は動かさず、下の正答を見て ○ / △ / × を記録します。</small></div><div class="review-answer-list">${qs.map((q,i)=>{const grade=pr.selfGrades[q.id]||'';return `<div class="review-answer-item"><b>${currentPage*PAGE_SIZE+i+1}.</b><div><span>${q.type==='manual'?'ChatGPT確認用':'正答'}：${esc(correctAnswerText(q))}</span>${q.guide?`<small>${esc(q.guide)}</small>`:''}</div>${q.type!=='manual'?`<div class="grade-row">${[['correct','○'],['partial','△'],['wrong','×']].map(([v,l])=>`<button class="grade-btn ${grade===v?'active '+v:''}" data-grade-qid="${q.id}" data-grade="${v}">${l}</button>`).join('')}</div>`:'<em>AI確認</em>'}</div>`}).join('')}</div>`;
}
...
function finishPackAction(){const pr=progress();if(!pr.reviewMode){pr.reviewMode=true;currentPage=0;pr.currentPage=0;save();renderPractice();scrollTo({top:0,behavior:'smooth'});toast('答え合わせモードに入りました');return}finalizeReview()}
